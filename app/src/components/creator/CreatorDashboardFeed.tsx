...
import { ArrowLeft, Plus, Upload, TrendingUp, Eye, Ticket, Clock, ChevronRight, Users, BarChart3, DollarSign, Trash2, Loader2 } from "lucide-react";
...
export default function CreatorDashboardFeed({ series, stats, isCreator }: CreatorDashboardFeedProps) {
  ...
  const handleDelete = async (seriesId: string, e: React.MouseEvent) => {
    ...
    if (deleteConfirm === seriesId) {
      ...
      try {
        const result = await apiDeleteSeries(seriesId);
        if (result.success) {
          router.refresh();
        } else {
          alert(result.error || "Устгахад алдаа");
        }
      } catch (err) {
      ...
      setDeleteConfirm(seriesId);
      // Auto-cancel after 3 seconds
      setTimeout(() => setDeleteConfirm((prev) => (prev === seriesId ? null : prev)), 3000);
    }
  };
  ...
  const sortedByViews = useMemo(() => [...series].sort((a, b) => b.viewsNum - a.viewsNum), [series]);
  const sortedByEarnings = useMemo(() => [...series].sort((a, b) => b.earnings - a.earnings), [series]);
                        ...
                        {deleteConfirm === s.id ? (
                          <>
                            <button
                              ...
                              className="text-[10px] bg-red-500 text-white px-2 py-1.5 rounded-lg font-semibold"
                            >
                              {deleting === s.id ? <Loader2 size={10} className="animate-spin" /> : "Устгах"}
                            </button>
                            <button
                              onClick={cancelDelete}
                              className="text-[10px] bg-white/10 text-white px-2 py-1.5 rounded-lg"
                            >
                              Үгүй
                            </button>
                          </>
